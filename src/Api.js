...
{
  return axios({ url: 'localhost:8081/game/' + encodeURIComponent(id) + '/teamA'
    , method: 'post'
    , data: body
    , responseType: 'json'
    });
...
var postGameByIdUndecided = function(id, body)
{
  return axios({ url: 'localhost:8081/game/' + encodeURIComponent(id) + '/undecided'
    , method: 'post'
    , data: body
    , responseType: 'json'
    });
...
var getAuthOpenidReturn = function(openid.identity)
{
  return axios({ url: 'localhost:8081/auth/openid/return' + '?openid.identity=' + encodeURIComponent(openid.identity)
    , method: 'get'
    });
}
